...
function mergeBusy(blocks: CalendarBusyBlock[]): CalendarBusyBlock[] {
  if (blocks.length === 0) return [];
  const sorted = [...blocks].sort(
    (a, b) => a.start.getTime() - b.start.getTime(),
  );
  const merged: CalendarBusyBlock[] = [{ ...sorted[0] }];
...
export function findFreeSlotsForDay(
  ...
  for (const block of inflated) {
    ...
    if (block.end > cursor) cursor = block.end > dayEnd ? dayEnd : block.end;
  }
...
function slot(start: Date, end: Date): FreeSlot {
  return {
    start,
    end,
...
export function trimSlotToMinutes(slot: FreeSlot, minutes: number): FreeSlot {
