...
export function loadConfig(path?: string): PlannerConfig {
  const configPath =
    path ?? join(__dirname, "..", "config", "default.json");
  const raw = readFileSync(configPath, "utf8");
  const parsed = JSON.parse(raw) as PlannerConfig;
  return parsed;
}

export function parseLocalTimeOnDate(
  date: string,
  hhmm: string,
  timeZone: string,
): Date {
...
export function listWeekdayDatesFrom(
  start: IsoDate,
