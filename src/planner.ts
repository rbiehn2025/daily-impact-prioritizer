...
function itemSortScore(item: RankedWorkItem): number {
  return (
    item.urgencyScore * 2 +
...
export function planWeek(
  ...
  workItems: RankedWorkItem[],
  config: PlannerConfig,
  horizonEnd: IsoDate,
): WeekPlan {
  const timeZone = config.timezone;
  const runDate = localDateString(runAt, timeZone) as IsoDate;
  const dates = listWeekdayDatesFrom(runDate, horizonEnd, timeZone);

  const sortedItems = [...workItems].sort(
    (a, b) => itemSortScore(b) - itemSortScore(a),
  );
  ...
  for (const date of dates) {
    ...
    for (const item of candidates) {
      if (blockCount >= config.slots.maxBlocksPerDay) break;
      const slotIdx = slots.findIndex(
        (s) => s.durationMinutes >= item.timeboxMinutes,
      );
      if (slotIdx === -1) {
        continue;
      }
      ...
      const remainingMinutes = Math.floor(
        (rawSlot.end.getTime() - remainingStart.getTime()) / 60_000,
      );
...
export function endOfWeekIso(from: IsoDate, timeZone: string): IsoDate {
  const probe = parseLocalTimeOnDate(from, "12:00", timeZone);
  const dow = new Intl.DateTimeFormat("en-US", {
    timeZone,
    weekday: "short",
  }).format(probe);
  const toFriday: Record<string, number> = {
    Mon: 4,
    Tue: 3,
    Wed: 2,
    Thu: 1,
